...
<%@include file="/LearningManagementSystem/admin/head.jsp"%>
		...
		<table width="1000">
			<tr>
				...
				<a href="/LMS/LearningManagementSystem/common/MVC/writeForm.do">글쓰기</a></td>
			</tr>
		</table>
				...
				<c:forEach var="article" items="${articleList}"> <!-- ArrayList에 n번째 인덱스의 값을 하나씩 꺼내서 article변수에 저장 -->
					<tr height="30">
						...
						<c:if test="${article.re_level > 0}"> <!-- 읽을 수 있는 프로퍼티명인지 확인. => getre_level()메소드 호출 -->
							...
							<a href="/LMS/LearningManagementSystem/common/MVC/content.do?num=${article.num}&pageNum=${currentPage}">${article.subject}</a> 
		...
		<c:if test="${count > 0}">
			...
			<c:if test="${startPage > 10}">
				<a href="/LMS/LearningManagementSystem/common/MVC/list.do?pageNum=${startPage - 10 }">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="/LMS/LearningManagementSystem/common/MVC/list.do?pageNum=${i}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount}">
				<a href="/LMS/LearningManagementSystem/common/MVC/list.do?pageNum=${startPage + 10}">[다음]</a>
			</c:if>
		</c:if>
